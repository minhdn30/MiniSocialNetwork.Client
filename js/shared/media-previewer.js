...
const MediaPreviewer = {
    ...
    async applyBackdropForItem(item, renderId) {
        ...
        const colorSource = isVideo
            ? (item?.thumbnailUrl || item?.ThumbnailUrl || item?.mediaUrl || item?.MediaUrl || '')
    ...
    getConversationId() {
        return this.normalizeId(this.options?.conversationId || this.options?.ConversationId);
    },

    getCurrentItem() {
        if (!Array.isArray(this.mediaList) || this.mediaList.length === 0) return null;
        if (!Number.isFinite(this.currentIndex)) return null;
        if (this.currentIndex < 0 || this.currentIndex >= this.mediaList.length) return null;
    ...
    _getItemStableKey(item, index) {
        const messageMediaId = this.normalizeId(item?.messageMediaId || item?.MessageMediaId);
    ...
    normalizeMediaList(mediaList) {
        ...
        return mediaList
            .map((item) => {
                ...
                const mediaType = item?.mediaType !== undefined
    ...
    isViewingMessage(messageId, options = {}) {
        ...
        if (currentItem && this._getItemMessageId(currentItem) === normalizedMessageId) return true;

        if (!checkAny) return false;
        return this.mediaList.some((item) => this._getItemMessageId(item) === normalizedMessageId);
    },
    ...
    syncAfterExternalListMutation(options = {}) {
        ...
        const previousIndex = Number.isFinite(this.currentIndex) ? this.currentIndex : 0;
        const previousItem = this.mediaList[previousIndex] || null;
        const previousKey = previousItem ? this._getItemStableKey(previousItem, previousIndex) : '';
        ...
        if (previousKey) {
            const matchedIndex = this.mediaList.findIndex((item, idx) => this._getItemStableKey(item, idx) === previousKey);
            if (matchedIndex >= 0) {
    ...
    async requestBoundaryMedia(direction) {
        const fn = direction === 'prev' ? this.options?.requestPrev : this.options?.requestNext;
        ...
        try {
            ...
            console.error(`MediaPreviewer request${direction === 'prev' ? 'Prev' : 'Next'} failed:`, err);
            return false;
        }
    },
    ...
    open(mediaList, startIndex = 0, options = null) {
        ...
        if (Number.isFinite(initialIndex)) {
            this.currentIndex = Math.min(Math.max(initialIndex, 0), this.mediaList.length - 1);
        } else {
            this.currentIndex = 0;
        }
    ...
    renderThumbs() {
        const mode = this.options?.thumbnailMode === 'windowed' ? 'windowed' : 'all';
        ...
        this.thumbsContainer.innerHTML = displayList.map((m, idx) => {
            const absoluteIndex = start + idx;
            const rawThumb = (m.thumbnailUrl || '').toString().trim();
            const hasUsableThumbnail = !!rawThumb && rawThumb.toLowerCase() !== 'null' && rawThumb.toLowerCase() !== 'undefined';
    ...
    setupVideoThumbFallbacks() {
        ...
        const thumbVideos = this.thumbsContainer.querySelectorAll('.preview-thumb-video');
        thumbVideos.forEach((thumbEl) => {
            ...
            const markImageLoaded = () => {
                const ok = Number(imageEl.naturalWidth) > 0 && Number(imageEl.naturalHeight) > 0;
                if (!ok) {
                    useVideoFallback();
                    return;
                }
